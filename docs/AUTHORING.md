...
**Persian.** Use نیم‌فاصله (ZWNJ) correctly: می‌کند، آن‌ها، نمی‌شود، closureها (ZWNJ before ها after a Latin word: `closure‌ها`). Persian punctuation «» for quotes, ، for commas in Persian sentences; Latin punctuation inside code. Numbers in prose may be Persian digits (۳٫۶) or Latin when they are literals (`3.6`); be consistent within a lesson. Register: explanatory, direct, second person allowed ("مقدار را همان لحظه capture کن"), no filler, no "در این ویدیو".
