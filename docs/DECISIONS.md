...
## 60. Optional Gemini relay infrastructure (`AI_PROXY_URL`), ported from EnglishOS — built on a misdiagnosis, not currently needed (2026-09-18, corrected same day)

**Original (incorrect) problem statement.** A first test of `GeminiClient` returned a 403 HTML page from Google's edge and was read as "Google blocks the Generative Language API from Iranian IPs entirely." **This was wrong** — that test called `generativelanguage.googleapis.com/v1beta/models` with no `x-goog-api-key` header at all; Google's edge shows that same generic block page for unauthenticated requests, which is not evidence of a regional block. Re-tested properly right after building the fix below: the real `GeminiClient::generateJson()` call (real API key, real model, real prompt) — direct, no proxy, no VPN — returned a normal 200 with real content. **Gemini was never actually broken in production**; `Evaluator` and `ReviewPracticeGenerator` were working the whole time.

**What was built anyway, and why it's being kept.** Before the re-test caught the mistake, the fix already built and running in the sibling project `EnglishOS` (`/var/www/EnglishOS`) was ported over: an optional HTTP relay. `GeminiClient` now goes through `App\Services\Ai\Concerns\UsesOutboundProxy` — when `services.ai_proxy.url` (`AI_PROXY_URL`) is set, the request is sent to that URL instead of Gemini directly, with the real destination in an `X-Relay-Url` header and a shared secret in `X-Relay-Auth`; **left blank (the default, and production's actual current state), behavior is exactly the plain direct call that was just confirmed working.** The relay script (`tools/ai-relay.py`, copied from `EnglishOS/scripts/ai-relay.py`, allowlist trimmed to just `generativelanguage.googleapis.com`) and its test (`UsesOutboundProxyTest`) are kept as dormant, zero-risk infrastructure — harmless since unused, and already there the day a real block (regional, per-key IP restriction, or otherwise) actually shows up, without needing another round-trip through this same investigation.

**Consequences.** No behavior change in production — `AI_PROXY_URL` was never set there, so nothing to undo. Full suite 128/128, `vendor/bin/pint --dirty` clean, both true before and after this correction. **Do not re-diagnose "Gemini is blocked from Iran" from a bare unauthenticated request to `/v1beta/models` again** — that specific test is misleading; always test through the real `GeminiClient` call (or at minimum, curl with the actual `x-goog-api-key` header) before concluding anything about reachability.
