...
- [ ] **Chosen next path (owner picked via `AskUserQuestion`, 2026-09-17): request SSH access from Parspack support**, then build a **reverse SSH tunnel** instead of Cloudflare Tunnel — plain SSH to the owner's own VPS isn't a known circumvention signature, so it's expected to survive the same DPI filtering that blocks Cloudflare Tunnel/ngrok. Port 22 on `45.159.149.29` is open and reachable (checked via `nc`/`/dev/tcp`), but the cPanel account itself has no SSH access enabled (owner confirmed "ssh ندارم") — a support ticket asking Parspack to enable "SSH Access" for the account is the next step, not yet sent as of this writing.
- [ ] **If Parspack grants SSH:** set up `autossh` (or a systemd unit with `ExecStart=ssh -N -R 18765:localhost:8765 ...` + `Restart=always`) on the owner's machine for a persistent `-R` reverse tunnel to the VPS, then an nginx/Apache vhost on the VPS proxying `media.growwise.ir` → `localhost:18765` (on the VPS itself), then update `MEDIA_BASE_URL=https://media.growwise.ir` in production `.env` and `/_ops/optimize`.
- [ ] **If Parspack refuses/can't grant SSH:** fall back to the other option surfaced to the owner — abandon the tunnel approach and actually host video files on the VPS's own disk (reverses §25's "media stays on the owner's machine" decision), pending a check of available disk quota on the shared hosting plan.
...
**Scope.** Only video `<source>` and caption `<track>` (the bandwidth-heavy pieces). Downloadable attachments (`lessons/show.blade.php`, `session/show.blade.php`'s `<a href="{{ media_url(...) }}" download>`) were left on `media_url()` only — an `<a>` has no natural "retry on failure" hook without extra JS, and attachments are small/low-traffic compared to video.
...
**Consequences.** No DB schema change — `media_download_url()` derives everything from the existing `/media/...`-prefixed URLs already stored on `LessonVideo`/subtitle records, same as `media_url()`. 2 new feature tests in `LessonPageTest` (fallback markup present when `download_base_url` is set, absent when it isn't); full suite 125/125. `vendor/bin/pint --dirty` clean. `npm run build` run to compile the `player.js` change. Owner still needs to: (1) actually provision the download host and set `MEDIA_DOWNLOAD_BASE_URL` in production `.env`, (2) upload videos there per-course as migration happens, (3) `/_ops/optimize` after the `.env` change. §53's SSH/reverse-tunnel path is still the fallback plan if this download-host approach doesn't pan out.
