...
## 60. Gemini calls blocked from Iran-hosted production — optional HTTP relay (`AI_PROXY_URL`), ported from EnglishOS (2026-09-18)

**Problem.** Tested `GeminiClient` directly (both locally and conceptually against production, which is also Iran-hosted on Parspack): `generativelanguage.googleapis.com` returns Google's own regional-block page (a 403 HTML page from Google's edge, not a normal API error) — Google blocks the Generative Language API from Iranian IPs entirely. This means `Evaluator` (grading every non-mcq practice) and `ReviewPracticeGenerator` (AI-authored review practices) were likely never actually working in production.

**Decision.** Ported the exact same fix already built and running in the sibling project `EnglishOS` (`/var/www/EnglishOS`): an optional HTTP relay. `GeminiClient` now goes through `App\Services\Ai\Concerns\UsesOutboundProxy` — when `services.ai_proxy.url` (`AI_PROXY_URL`) is set, the request is sent to that URL instead of Gemini directly, with the real destination in an `X-Relay-Url` header and a shared secret in `X-Relay-Auth`; left blank (the default), behavior is unchanged. The relay itself (`tools/ai-relay.py`, copied from `EnglishOS/scripts/ai-relay.py`, allowlist trimmed to just `generativelanguage.googleapis.com` since this project has no Groq/Pexels calls) is a small Python HTTP server run on a machine that *can* reach Gemini (e.g. behind a VPN), forwarding whatever it's told to via those two headers — plain HTTP request/response, not a real CONNECT-based forward proxy, so it can be exposed over a simple HTTP tunnel like a free `localhost.run` SSH tunnel rather than needing paid tunnel infrastructure.

**Why.** Direct owner request after the Iran-block was found; rather than design something new, replicate the already-proven fix from `EnglishOS`, which solves the identical problem (same host, same Google region-block) for the same `GeminiClient` shape.

**Consequences.** No schema/behavior change when `AI_PROXY_URL` is unset — every existing test and the local dev flow keep working exactly as before. 2 new tests (`UsesOutboundProxyTest`, ported from `EnglishOS`'s identical test) cover the trait directly; full suite 128/128; `vendor/bin/pint --dirty` clean. **Not yet wired up live** — the owner still needs to: (1) run `tools/ai-relay.py` on a machine that can reach Google (their laptop, while its VPN is on), (2) expose it via an HTTP tunnel (`ssh -R 80:127.0.0.1:8899 nokey@localhost.run`, matching `EnglishOS`'s approach), (3) set `AI_PROXY_URL`/`AI_PROXY_SECRET` in production `.env` to that tunnel's URL and the relay's `RELAY_SECRET`, (4) `/_ops/optimize`. Same caveat as `EnglishOS`'s setup: the relay machine needs its VPN (or other route around the block) active whenever a grading call happens — an AI-graded practice attempt or a review-practice generation will fail if the relay machine is offline/VPN-off at that moment, not a persistent-uptime requirement like §53's rejected video-tunnel-over-VPN idea, just an intermittent one.
