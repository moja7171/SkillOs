...
## 60. Gemini is genuinely blocked from this network without a VPN — relay infrastructure (`AI_PROXY_URL`), ported from EnglishOS (2026-09-18, went through two wrong turns before landing here)

**Two misdiagnoses on the way to this, both worth recording so they aren't repeated:**

1. First test: called `generativelanguage.googleapis.com/v1beta/models` with no `x-goog-api-key` header, got Google's generic 403 block page, wrongly read as "Google blocks this region entirely." That specific test is meaningless — Google shows the same generic block page for any unauthenticated request, proving nothing about reachability with a real key.
2. Second test (the "correction"): retested with the real API key and got a clean 200 — direct, seemingly no proxy/VPN needed. **This was also wrong**, because the owner's VPN (`Happ`, system-wide TUN via `sing-box` run as root) was still active on the machine at the time; "direct" (no `proxychains`) still meant "through the VPN's transparent system-wide routing," not truly direct. The mistake: not re-checking VPN process state (`ps aux | grep -E "xray|sing-box"`) immediately before a test whose entire point is measuring reachability *without* a VPN.

**Confirmed, carefully this time (owner explicitly turned the VPN off, `ps aux` verified no `xray`/`sing-box` process running, `ss` verified the SOCKS port closed, *then* tested):** the real `GeminiClient`-equivalent call (real key, real model, real prompt, via plain `curl`) returns a **403 Google region-block page** with the VPN confirmed off, and a normal 200 with the VPN confirmed on. **This is a genuine block, not a testing artifact** — `Evaluator` (grading every non-mcq practice) and `ReviewPracticeGenerator` do need a way around it in production, which doesn't have a VPN of its own.

**Decision (unchanged from the first pass, now correctly justified):** the relay built earlier this same day — `App\Services\Ai\Concerns\UsesOutboundProxy` (ported from the sibling project `EnglishOS`'s identical mechanism) routes `GeminiClient` through `AI_PROXY_URL` when set, with the real destination in `X-Relay-Url` and a shared secret in `X-Relay-Auth`; unset, behavior is a plain direct call. `tools/ai-relay.py` (ported from `EnglishOS/scripts/ai-relay.py`, allowlist trimmed to just `generativelanguage.googleapis.com`) is the relay server, meant to run on a machine that can reach Google (e.g. the owner's laptop, VPN on), exposed via a plain HTTP tunnel (`localhost.run`, free, no CONNECT/raw-TCP needed since the relay speaks plain HTTP).

**Status: code is in place, not yet wired up live.** Owner still needs to, same as documented the first time: (1) run `tools/ai-relay.py` on a VPN-connected machine, (2) expose it via `ssh -R 80:127.0.0.1:8899 nokey@localhost.run`, (3) set `AI_PROXY_URL`/`AI_PROXY_SECRET` in production `.env`, (4) `/_ops/optimize`. Until that's done, every AI-graded (non-mcq) practice attempt and every review-practice generation fails in production.

**Consequences / lesson for next time.** Full suite 128/128, `vendor/bin/pint --dirty` clean — true throughout, since none of this touched behavior when `AI_PROXY_URL` is unset. **Process lesson, not just a content correction:** when a "reachability" test is specifically about *whether a VPN/proxy is needed*, verify the VPN's actual process state immediately before the test that's supposed to prove it's not needed — a system-wide TUN VPN makes "I didn't use proxychains" an unreliable signal for "this was a direct connection."

---

## 61. Gemini relay, take two: VPS instead of laptop — laptop/localhost.run path abandoned after it broke EnglishOS's live tunnel (2026-09-18)

**What was tried first and why it's abandoned.** Set up the full laptop-based path from §60 as a persistent setup: `tools/ai-relay.py` run as a user systemd service, exposed via a `localhost.run` SSH tunnel (also a systemd service, `Restart=always`), plus a 5-minute healthcheck timer — mirroring EnglishOS's own now-superseded laptop arrangement move-for-move (dedicated SSH key, `~/.config/systemd/user/*.service`). **This broke EnglishOS's already-running production tunnel**: both projects' tunnels originate from the same laptop (same public IP once the VPN is on), and `localhost.run` appears to key its assigned subdomain off the source IP for anonymous/unregistered-enough connections — starting SkillOS's tunnel silently hijacked EnglishOS's existing `e428e88fe30e57.lhr.life` registration, breaking its relay for real production traffic until EnglishOS's tunnel service was force-restarted. A `-R <name>:80:...` named-subdomain request (to sidestep the collision without an account) failed outright ("unknown error") — named subdomains need a registered `admin.localhost.run` account, which EnglishOS's tunnel already has and SkillOS's fresh key doesn't share.

**Fix applied immediately:** `systemctl --user disable --now skillos-ai-tunnel.service skillos-ai-tunnel-healthcheck.timer skillos-ai-relay.service` — confirmed EnglishOS's relay round-trips correctly again before doing anything else. These three unit files, the `skillos_relay_tunnel` SSH keypair, and `/home/moja/bin/skillos-relay.env` are left in place but disabled/unused (harmless, not auto-starting) rather than deleted, in case the account-sharing/named-subdomain route is revisited later.

**Decision: move straight to the same permanent fix EnglishOS itself has already moved to** — a small, dedicated, always-on VPS outside Iran, not the owner's own laptop. No shared-source-IP collision risk (a VPS has its own IP), no dependency on the owner's laptop/VPN being on, no localhost.run subdomain churn to babysit. Ported `EnglishOS/scripts/vps-relay-setup.sh` and its Persian README to `tools/vps-relay-setup.sh` / `tools/vps-relay-README.fa.md` (service/user/paths renamed `skillos-*`/`/opt/skillos-relay`, allowlist already trimmed to just Gemini per §60). Script is idempotent (systemd unit + Caddy + ufw, `<public-ip>.sslip.io` for a free auto-HTTPS hostname with zero DNS setup), same as EnglishOS's proven version.

**Also added: `/_ops/ai-check`**, an `OpsController` action that round-trips a real `GeminiClient::generateJson()` call (direct or through the relay, whichever `AI_PROXY_URL` currently resolves to) and reports `Gemini relay test SUCCESS`/`FAILED` — lets the owner confirm a `.env` change actually works without needing a learner to submit a real practice. `status` also now prints the current `ai proxy:` config line. Mirrors the "AI proxy relay diagnostic" block EnglishOS's own `deploy.php` has.

**Consequences.** 2 new tests (`test_ops_ai_check_reports_success_on_a_real_gemini_round_trip`, `..._failure_when_gemini_errors`, `Http::fake`d, no real network call in tests); full suite 130/130; `vendor/bin/pint --dirty` clean. **Not yet live** — the owner still needs to: (1) actually provision a small foreign VPS, (2) `scp` `tools/ai-relay.py` and `tools/vps-relay-setup.sh` there and run the setup script (see `tools/vps-relay-README.fa.md`), (3) set `AI_PROXY_URL`/`AI_PROXY_SECRET` in production `.env` to the script's output, (4) `/_ops/optimize`, (5) confirm via `/_ops/ai-check?token=...`. Until then, `AI_PROXY_URL` stays unset in production and every AI-graded practice / review-practice generation still fails (§60's finding).

---

## 62. Two owner-reported bugs: answer auto-revealed before "give up" clicked, and spacebar/pause crossing between videos in multi-video lessons (2026-09-22)

**Bug 1 — answer shown without an explicit give-up.** `AttemptSession::submit()` (§48/original design, documented in this file at line 148 and in the class docblock) auto-finalized the attempt as `incorrect` and set `evidence.revealed = true` the moment both hints (`MAX_HINTS = 2`) were exhausted and a third wrong answer came in — before the learner ever clicked "بی‌خیال، جواب رو نشون بده" (`giveUp()`). Owner wants the answer to only ever appear via that explicit click. **Fix:** the hint-exhausted branch in `submit()` now just saves feedback/history and keeps the attempt open (`result_status` stays `started`) — the learner can keep retrying indefinitely; only `giveUp()` sets `revealed` and finalizes. `SessionTest::test_wrong_answers_reveal_hints_then_the_answer` renamed to `test_wrong_answers_reveal_hints_but_not_the_answer` and extended to assert the attempt stays open with the answer hidden after hint exhaustion, then reveals only after an explicit `session.give-up` POST.

**Bug 2 — spacebar/pause crossing between videos in a multi-video lesson.** `lesson-content.blade.php` mounts a separate Plyr instance for every video in a lesson up front (all `.js-player` elements exist in the DOM at once; only the active one is shown via `x-show`/inline `display:none`). Each instance was configured `keyboard: { global: true }`, which makes Plyr attach its keydown/keyup handler on `window` with no check for which player is actually visible — so one spacebar press called `togglePlay()` on every mounted player at once, and the picker's `pick()` handler only paused `<video>` elements directly, not Plyr's own state. **Fix:** `resources/js/player.js` now tracks mounted Plyr instances and exposes `setActiveVideo(groupEl, activeIndex)`, which uses Plyr's own (undocumented but public) `player.listeners.global(bool)` toggle to keep the page-wide keyboard listener live on only the active video per group; the video-picker wrapper gets `data-video-group`, and `pick()` calls `window.setActiveVideo($el, i)` after switching. Single-video lessons are unaffected (still one active listener).

**Consequences.** `vendor/bin/pint --dirty` clean; full suite 129/130 (only the pre-existing, unrelated `DeployTest::test_ops_ai_check_reports_success_on_a_real_gemini_round_trip` live-network failure from §61/62, not touched here); `npm run build` clean. Bug 2 has no automated coverage (no JS test harness in this repo) — verify live: open a lesson with 2+ videos, switch to video 2 or 3, press space, confirm only that video toggles.

---

## 63. "انجام دادم" gated on watching every video, not just passing practices — plus clearer multi-video UI (2026-09-22)

**Problem.** In a multi-video lesson, learners sometimes didn't notice a lesson had more than one video (the only signal was a row of unlabeled 28px numbered buttons) and could hit "انجام دادم" having only watched the first one — the gate (`Lesson::allPracticesPassedBy()`) only checked practices, never videos.

**Decision.** New `video_views` table (`user_id`, `lesson_video_id`, `watched_at`, unique per pair) — DB-backed, not localStorage, so the gate is enforced server-side the same way §43 established for the practices gate ("not just a hidden client-side button"). Keyed on `lesson_video_id`, the same volatile-across-reimport identity `player.js`'s existing position-memory feature already relies on (`CourseImporter::import()` does `$lesson->videos()->delete()` then recreates on every import) — an accepted tradeoff carried over, not a new one; a re-import resets watched history same as it already resets scrub position.

`player.js`'s Plyr `ended` handler (previously only reset the saved scrub position to `'0'`) now also POSTs to the new `lesson-videos/{lessonVideo}/watched` route (`VideoViewController::store`, `updateOrCreate` so re-watching is idempotent) and dispatches a `window` `video-watched` CustomEvent (`{videoId}`) so any open picker/mark-done UI updates its checkmark live without a reload.

`Lesson::watchedVideoIdsBy()` / `allVideosWatchedBy()` added, mirroring `allPracticesPassedBy()`. `LessonController::markDone()` now `abort_unless`s both. The button itself stays **enabled** once practices pass (owner's call, unlike the practices gate which disables outright) — clicking with videos still unwatched fires a client-side check (`x-data` on the mark-done card, tracking `watchedVideoIds` + listening for `video-watched`) that blocks the submit and shows an inline `.error`-styled warning instead, matching the "clickable, warns instead of silently disabled" UX the owner asked for specifically for this case. Server-side 422 stays the backstop for a stale page or a direct request.

**Multi-video clarity**, per the owner's complaint that it wasn't obvious a lesson had more than one video: the "ویدیو" tab button now carries a "N قسمت" badge whenever a lesson has >1 video; the picker gets a static caption line ("این درس N ویدیو داره — بعد از هرکدوم برو سراغ بعدی") above the numbered buttons; and each picker button swaps its number for a check icon (`text-ok`) once that video's been watched to the end.

**Consequences.** New table + `VideoView` model; `vendor/bin/pint --dirty` clean; full suite 131/132 (only the pre-existing unrelated `DeployTest` live-Gemini failure from §61); `npm run build` clean. `LessonMarkDoneTest` gained `test_mark_done_is_rejected_until_every_video_is_watched` and `test_watching_the_same_video_twice_does_not_duplicate_the_view_row`. Not covered by any automated test: the live checkmark update and the inline warning-on-click UI (both pure Alpine/JS, no JS test harness in this repo) — verify live.

---

## 64. No more essay-style practices — every non-mcq, non-coding practice must be a true short answer (owner rule, 2026-09-22, general going forward; migration starts with `requirements-engineering`)

**Problem.** Reviewing `requirements-engineering`'s practices, the owner found `explanation` and `scenario` forms had drifted into multi-sentence essay answers graded by Gemini against a rubric — and even nominally-`short_answer` practices in the same course had the same multi-sentence shape (no schema/UI ever enforced brevity; `Evaluator` grades every non-mcq form identically via rubric, and `session/show.blade.php` renders the same 8-row textarea for `short_answer`/`explanation`/`scenario` alike, so nothing technical was stopping this drift).

**Decision.** Going forward, any practice whose answer would take more than a short phrase must be rewritten as a true `short_answer`: a specific fact/term/name/number/reason recalled in 1–2 words, not a paragraph of reasoning. A prompt may ask for more than one such short answer (e.g. two or three named items) when that's natural, but never a "explain in 3–4 sentences" style prompt. `explanation` and `scenario` are retired for new/rewritten authoring; the schema/validator (`FORMS` in `tools/authoring/validate.py`, `Activity::FORM_LABELS`) is left untouched for now since other courses (`complete-python-mastery`, `python-deep-dive-1`, etc.) still use both forms extensively and aren't being migrated yet — only remove/rename a form value once every course using it has been migrated.
